...
class EmployeesController extends Controller
{
	...
	public function __construct() {
		$this->menu_id = Menu::get('Employees');
	...
	{
		if(Menu::hasAccess("Employees", "create")) {
	...
	{
		if(Menu::hasAccess("Employees", "view")) {
	...
	{
		if(Menu::hasAccess("Employees", "edit")) {
	...
	{
		if(Menu::hasAccess("Employees", "delete")) {
	...
	public function change_password($id, Request $request) {
		...
		if(env('MAIL_USERNAME') != null && env('MAIL_USERNAME') != "null" && env('MAIL_USERNAME') != "") {
			...
			Mail::send('emails.send_login_cred_change', ['user' => $user, 'password' => $request->password], function ($m) use ($user) {
				...
				$m->to($user->email, $user->name)->subject('RAB ERP - Login Credentials chnaged');
			});
		} else {
