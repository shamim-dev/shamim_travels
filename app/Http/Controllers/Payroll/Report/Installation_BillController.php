...
class Installation_BillController extends Controller
{
    public $show_action = true;

    public function __construct()
    {
        $this->menu_id = Menu::get('Installation_Bill');
    }
    ...
    public function index()
    {
        if(Menu::hasAccess($this->menu_id)) {
            ...
            return View('payroll.report.common.common_salary_report_header', [
                ...
                'header'=>'Installation Bill',
                'is_officer'=>1,
                'print_url' => 'installation_bill_pdf',
            ]);
    ...
    public function installation_bill_pdf(Request $request)
    {
        //echo 'munna';die();
        if(Menu::hasAccess("Installation_Bill", "create"))
        {
            //.................common input start..................
            ...
            //..........Designations start......
            $designations=DB::SELECT("SELECT d.id,d.designation_name, COUNT( d.id ) as sum_designation
            FROM  `payroll_salary_process` psp
            INNER JOIN posting_record pr ON ( pr.`posting_rec_id` = psp.`posting_rec_id` ) 
            INNER JOIN employees_info ei ON ( ei.emp_id = pr.emp_id ) 
            INNER JOIN ranks r ON ( ei.rank_id = r.id ) 
            ...
            GROUP BY d.id
            ORDER BY d.designation_level ASC 
            ");
            ...
            $mergeData=[];
            $pdf = PDF::loadView('payroll.report.installation_bill',$data,$mergeData,$config);
            //return $pdf->stream('installation_bill_'.date('Y-m-d H:i:s').'.pdf');
            return $pdf->download('installation_bill_'.date('Y-m-d H:i:s').'.pdf');
            //................report pdf end.............
        }
    }
