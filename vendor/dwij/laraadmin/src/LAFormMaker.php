...
class LAFormMaker
{
	...
	**/
	public static function input($module, $field_name, $default_val = null, $required2 = null, $class = 'form-control', $params = [])
	{
		...
		//if(Module::hasFieldAccess($module->id, $module->fields[$field_name]['id'], $access_type = "write")) {
			...
			if($required && !isset($params['required'])) {
				...
				$required_ast = "<span class='la-required'>* </span>";
			}
			
			switch ($field_type->name) {
				case 'Address':
					// $out .= '<label for="'.$field_name.'">'.$label.$required_ast.' :</label>';
					$out .= '<label for="'.$field_name.'">'.Lang::get('messages.'.$label).$required_ast.':</label>';
					//echo Lang::get('messages.'.$Save);
				...
				case 'Date':
					...
					$out .= "<div class='input-group date' id='dp_".$field_name."'>";
				...
				case 'Datetime':
					...
					$params['id']=$field_name;
					$out .= "<div class='input-group datetime' id='dp_".$field_name."'>";
					$out .= Form::text($field_name, $dval, $params);
					$out .= "<span class='input-group-addon'><span class='fa fa-calendar'></span></span></div>";
					break;
				case 'Decimal':
				...
				case 'Email':
					...
					break;
				case 'File':
					$out .= '<label for="'.$field_name.'" style="display:block;">'.Lang::get('messages.'.$label).$required_ast.':</label>';
				...
				case 'Image':
					...
					if(isset($upload->id)) {
						$out .= "<a class='btn btn-default btn_upload_image hide' file_type='image' selecter='".$field_name."'>Upload <i class='fa fa-cloud-upload'></i></a>
							<div class='uploaded_image'><img src='".url("files/".$upload->hash.DIRECTORY_SEPARATOR.$upload->name)."'><i title='Remove Image' class='fa fa-times'></i></div>";
					} else {
	...
	/**
	* Print input field enclosed within form-group double column for edit 
	**/
	public static function edit_input($module, $field_name, $default_val = null, $required2 = null, $class = 'form-control', $params = [])
	{
		// Check Field Write Aceess
		//if(Module::hasFieldAccess($module->id, $module->fields[$field_name]['id'], $access_type = "write")) {
			...
			switch ($field_type->name) {
				case 'Address':
					$out .= '<div class="col-md-2"><div class="form-group"><label for="'.$field_name.'">'.Lang::get('messages.'.$label).$required_ast.':</label></div></div>';
					...
					$params['rows'] = 3;
					$params['id']=$field_name;
					$out .= '<div class="col-md-4"><div class="form-group">'.Form::textarea($field_name, $default_val, $params).'</div></div>';
					break;
				case 'Checkbox':
					$out .= '<div class="col-md-2"><div class="form-group"><label for="'.$field_name.'">'.Lang::get('messages.'.$label).$required_ast.':</label></div></div>';
					$out .= '<input type="hidden" value="false" name="'.$field_name.'_hidden">';
				...
				case 'Currency':
					...
					$params['min'] = "0";
					$params['id']=$field_name;
					$out .= '<div class="col-md-4"><div class="form-group">'.Form::number($field_name, $default_val, $params).'</div></div>';
					break;
				case 'Date':
					$out .= '<div class="col-md-2"><div class="form-group"><label for="'.$field_name.'">'.Lang::get('messages.'.$label).$required_ast.':</label></div></div>';
					...
					$out .= "<div class='col-md-4'><div class='form-group'><div class='input-group date' id='dp_".$field_name."'>";
					$out .= Form::text($field_name, $dval, $params);
					$out .= "<span class='input-group-addon'><span class='fa fa-calendar'></span></span></div></div></div>";
					// $out .= Form::date($field_name, $default_val, $params);
					break;
				case 'Datetime':
					$out .= '<div class="col-md-2"><div class="form-group"><label for="'.$field_name.'">'.Lang::get('messages.'.$label).$required_ast.':</label></div></div>';
					...
					$out .= "<div class='col-md-4'><div class='form-group'><div class='input-group datetime' id='dp_".$field_name."'>";
					$out .= Form::text($field_name, $dval, $params);
					$out .= "<span class='input-group-addon'><span class='fa fa-calendar'></span></span></div></div></div>";
					break;
				case 'Decimal':
				...
				case 'Dropdown':
					...
					$params['id']=$field_name;
					$out .= '<div class="col-md-4"><div class="form-group">'.Form::select($field_name, $popup_vals, $default_val, $params).'</div></div>';
					break;
				case 'Email':
					$out .= '<div class="col-md-2"><div class="form-group"><label for="'.$field_name.'">'.Lang::get('messages.'.$label).$required_ast.':</label></div></div>';
					...
					$params['data-rule-email'] = "true";
					$params['id']=$field_name;
					$out .= '<div class="col-md-4"><div class="form-group">'.Form::email($field_name, $default_val, $params).'</div></div>';
					break;
				case 'File':
					$out .= '<div class="col-md-2"><div class="form-group"><label for="'.$field_name.'" style="display:block;">'.Lang::get('messages.'.$label).$required_ast.':</label></div></div>';
					...
					if(isset($upload->id)) {
						$out .= "<div class='col-md-2'><div class='form-group'><a class='btn btn-default btn_upload_file hide' file_type='file' selecter='".$field_name."'>Upload <i class='fa fa-cloud-upload'></i></a>
							<a class='uploaded_file' target='_blank' href='".url("files/".$upload->hash.DIRECTORY_SEPARATOR.$upload->name)."'><i class='fa fa-file-o'></i><i title='Remove File' class='fa fa-times'></i></a></div></div>";
					} else {
						$out .= "<div class='col-md-2'><div class='form-group'><a class='btn btn-default btn_upload_file' file_type='file' selecter='".$field_name."'>Upload <i class='fa fa-cloud-upload'></i></a>
							<a class='uploaded_file hide' target='_blank'><i class='fa fa-file-o'></i><i title='Remove File' class='fa fa-times'></i></a></div></div>";
					}
					break;
				...
				case 'HTML':
					...
					// ############### Remaining
					$out .= '<div class="htmlbox" id="htmlbox_'.$field_name.'" contenteditable>'.$default_val.'</div>';
					$out .= '<div class="col-md-4"><div class="form-group">'.Form::hidden($field_name, $default_val, $params).'</div></div>';
					break;
				case 'Image':
					$out .= '<label for="'.$field_name.'" style="display:block;">'.Lang::get('messages.'.$label).$required_ast.':</label>';
				...
				case 'Mobile':
					...
					$out .= '<div class="col-md-4"><div class="form-group">'.Form::text($field_name, $default_val, $params).'</div></div>';
					break;
				case 'Multiselect':
					$out .= '<div class="col-md-2"><div class="form-group"><label for="'.$field_name.'">'.Lang::get('messages.'.$label).$required_ast.':</label></div></div>';
				...
				case 'Name':
					...
					break;
				case 'Password':
					$out .= '<div class="col-md-2"><label for="'.$field_name.'">'.Lang::get('messages.'.$label).$required_ast.':</label></div></div>';
					$params['id']=$field_name;
					$out .= '<div class="col-md-4"><div class="form-group">'.Form::password($field_name, $params).'</div></div>';
					break;
				case 'Radio':
					$out .= '<div class="col-md-2"><div class="form-group"><label for="'.$field_name.'">'.Lang::get('messages.'.$label).$required_ast.': </label></div></div>';
					...
					if(starts_with($popup_vals, "@")) {
						$popup_vals = LAFormMaker::process_values($popup_vals);
						$out .= '<div class="col-md-4"><div class="form-group">';
						foreach ($popup_vals as $key => $value) {
							$sel = false;
							if($default_val != "" && $default_val == $value) {
								$sel = true;
				...
				case 'Taginput':
					...
					$default_val = LAFormMaker::process_values($default_val);
					$out .= '<div class="col-md-4"><div class="form-group">'.Form::select($field_name."[]", $default_val, $default_val, $params).'</div></div>';
					break;
				case 'Textarea':
					$out .= '<div class="col-md-2"><div class="form-group"><label for="'.$field_name.'">'.Lang::get('messages.'.$label).$required_ast.':</label></div></div>';
	...
	**/
	...
	public static function process_values($json) {
		...
		if(is_string($json) && starts_with($json, "@")) {
			...
			//echo $table_name;die();
			if($table_name=="employees_infos"){
				$module_name="Employee_Info";
				$out = Module::getDDArrayRabId($module_name);
			}else{
				// Search Module
	...
	**/
	public static function display($module, $field_name, $class = 'form-control')
	{
		...
		//if(Module::hasFieldAccess($module->id, $module->fields[$field_name]['id'], $access_type = "view")) {
			...
			$out .= '<label for="'.$field_name.'" class="col-md-2">'.Lang::get('messages.'.$label).':</label>';
			...
			switch ($field_type->name) {
				...
				case 'Image':
					...
					if(isset($upload->id)) {
						$value = '<a class="preview" target="_blank" href="'.url("files/".$upload->hash.DIRECTORY_SEPARATOR.$upload->name).'"><img src="'.url("files/".$upload->hash.DIRECTORY_SEPARATOR.$upload->name).'"></a>';
					} else {
	...
	**/
	...
	// }
	public static function la_access($menu_id, $access_type = "view")   //menuid or menu name
	{
		return Menu::hasAccess($menu_id, $access_type);
	}
