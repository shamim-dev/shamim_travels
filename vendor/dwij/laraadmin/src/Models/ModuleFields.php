...
class ModuleFields extends Model
{
    ...
    public static function createField($request) {
        ...
        if(!isset($field->id)) {
            ...
            //.........default value null check start.....
            //$field->defaultvalue = $request->defaultvalue;
            if($request->defaultvalue=='' || $request->defaultvalue=='NULL' || $request->defaultvalue=='null' || $request->defaultvalue=='Null'){
                $field->defaultvalue =Null;
            }else{
                $field->defaultvalue =$request->defaultvalue;
            }
            //.........default value null check end.....
	...
	public static function getFieldValue($field, $value) {
		...
		if(Schema::hasTable($external_table_name)) {
            if($external_table_name=='employees_info'){
                $external_value = DB::table($external_table_name)->where('emp_id', $value)->get();

            }else if($external_table_name=='vehicle_infos'){
			...
			if(isset($external_value[0])) {
                ...
                if(isset($external_module->view_col)) {
                    ...
                    }else if(isset($external_value[0]->{"rab_id"})) {
                        return $external_value[0]->rab_id;
                    }else if(isset($external_value[0]->{"registration_no"})) {
                        return $external_value[0]->registration_no;
                    }
                }
			} else {
				return $value;
