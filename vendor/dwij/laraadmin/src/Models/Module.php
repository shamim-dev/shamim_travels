...
class Module extends Model
{
		...
		switch ($field->field_type) {
			...
			case 'Radio':
				...
				if(is_array($popup_vals)) {
					if($update) {
						$var = $table->string($field->colname)->change();
						// DB::statement("ALTER TABLE battalions CHANGE COLUMN battalion_status battalion_status ENUM('admin', 'user', 'candidate')");
						//$var =$table->enum($field->colname, $popup_vals);
	...
	**/
	...
	public static function getDDArrayRabId($module_name){
		$model = "App\\Models\\".$module_name;
		$result = $model::all();
		$out = array();
		foreach ($result as $row) {
			$out[$row->emp_id] = $row->rab_id;
		}
		return $out;
	}
	...
	**/
	public static function setDefaultRoleAccess($module_id, $menu_id, $role_id, $access_type = "readonly") {
		$module = Module::find($module_id);
		$module = Module::get($module->name);
		...
		$module_perm = DB::table('role_menu')->where('role_id', $role->id)->where('menu_id', $menu_id)->first();
		if(!isset($module_perm->id)) {
			DB::insert('insert into role_menu (role_id, menu_id, acc_view, acc_create, acc_edit, acc_delete, created_at, updated_at) values (?, ?, ?, ?, ?, ?, ?, ?)', [$role->id, $menu_id, $access_view, $access_create, $access_edit, $access_delete, $now, $now]);
		} else {
			DB::table('role_menu')->where('role_id', $role->id)->where('menu_id', $menu_id)->update(['acc_view' => $access_view, 'acc_create' => $access_create, 'acc_edit' => $access_edit, 'acc_delete' => $access_delete]);
		}
	}
